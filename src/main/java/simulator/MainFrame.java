...
class Transmissor extends Thread {
    ...
    public void run() {
        try {
            ...
            MainFrame.mainFrame.date1 = dateFormat.parse(MainFrame.mainFrame.DatahoraInicio_text.getText());
            ...
            while (continuar) {
                ...
                try {
                    ...
                    Double d_temperatura = 30.0D + 10.0D * (1.0D
                            + (double) Math.sin(Math.PI * (1D / 16D) * Double.parseDouble(Long.toString((long) MainFrame.mainFrame.date1.getTime() / 1000))));
                    Double d_umidade = 65.0D + 15.0D * (1.0D
                            + (double) Math.sin(Math.PI * (1D / 16D) * Double.parseDouble(Long.toString((long) MainFrame.mainFrame.date1.getTime() / 1000))));

                    MainFrame.mainFrame.textArea.append("serial:" + serial
                    ...
                    String requestData
                            ...
                            + "&datahora=" + URLEncoder.encode(dateFormat.format(MainFrame.mainFrame.date1), "UTF-8")
                            + "&serial=" + Long.toString(serial);

                    LocalDateTime ldt = LocalDateTime.ofInstant(MainFrame.mainFrame.date1.toInstant(), ZoneId.systemDefault());
                    LocalDateTime dateTime = ldt.plus(Duration.of(Integer.parseInt(MainFrame.mainFrame.deltaTvirtual_text.getText()),
                                    ChronoUnit.SECONDS));
